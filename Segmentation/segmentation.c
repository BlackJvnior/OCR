...
void center_letter(struct letter *src, double dst[])
{
    ...
    for (int i = 0; i < src->width; i++)
    {
        ...
        {
            dst[(start_x + i) + (start_y + j) * 32] = src->mat[(bot_x + i) + (bot_y + j) * 32];
        }
    }
}
...
{ //Boxes every letter of the text
    struct letter **list = malloc(sizeof(struct letter) * get_number_letters(img, cols));
    int y;
        ...
        if (lines[y] == 2)
        {
            for (; cols[index] != -42;)
            {
                ...
                for (int tmp_y = y; stop_checking != 1; tmp_y--)
                {
                    ...
                    {
                        pxl = get_pixel(img, check_x, tmp_y);
                        SDL_GetRGB(pxl, img->format, &pxlcolor, &pxlcolor, &pxlcolor);
                        if (pxlcolor == 0)
                        {
...
{ //Draws a line on img, at height y, between points s_x and end_x
    for (; start_x < end_x; start_x++)
        put_pixel(img, start_x, y, SDL_MapRGB(img->format, 255, 0, 0));
    return (img);
}
...
{ //Draws a column on img, at width x, between points s_y and end_y
    for (; start_y < end_y; start_y++)
        put_pixel(img, x, start_y, SDL_MapRGB(img->format, 255, 0, 0));
    return (img);
}
                           ...
                           SDL_Surface *img)
{
    ...
    for (going_up = botright_y - 1; line_has_black != 0; going_up--)
    {
        ...
        {
            pxl = get_pixel(img, tmp, going_up);
            SDL_GetRGB(pxl, img->format, &pxlcolor, &pxlcolor, &pxlcolor);
            if (pxlcolor == 0)
            {
                line_has_black = 1;
...
void binarize_letter(SDL_Surface *img, struct letter *l)
{
    ...
    for (int i = l->coord_y[0]; i < l->coord_y[1]; i++)
    {
        ...
        {
            SDL_GetRGB(get_pixel(img, j, i), img->format, &r, &g, &b);
            if (r == 0)
